...
// import 'package:url_launcher/url_launcher.dart'; // إذا أردت تفعيل الاتصال
...
class _TenantContractsScreenState extends State<TenantContractsScreen> {
  ...
  Future<void> _requestTermination(String contractId) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text("Are you sure you want to request contract termination? This will notify the landlord."),
        actions: [
    ...
    final (ok, msg) = await ApiService.updateContractStatus(contractId, 'terminated');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("My Contracts"), backgroundColor: const Color(0xFF00695C), elevation: 0),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF00695C)))
          : _contracts.isEmpty
              ? const Center(child: Text("No contracts found.", style: TextStyle(color: Colors.grey)))
              : ListView.builder(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                Text("Contract #${c['_id'].toString().substring(c['_id'].toString().length - 6)}", 
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Expanded(child: Text(property['address'] ?? "No Address", style: const TextStyle(color: Colors.grey))),
                                  ],
                                ),
                                ...
                                Row(
                                  children: [
                                    ...
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        const Text("Landlord", style: TextStyle(fontSize: 10, color: Colors.grey)),
                                        Text(landlord['name'] ?? "Unknown", style: const TextStyle(fontWeight: FontWeight.bold)),
                                      ],
                                    ),
                                    const Spacer(),
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Contacting ${landlord['name']}...")));
                                      },
                                    )
                                  ],
                                ),
                                ...
                                Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: [
                                    _infoItem("Start Date", DateFormat('yyyy-MM-dd').format(DateTime.parse(c['startDate']))),
                                    _infoItem("End Date", DateFormat('yyyy-MM-dd').format(DateTime.parse(c['endDate']))),
                                    _infoItem("Rent", "\$${c['rentAmount']}", isPrice: true),
                                  ],
                                ),
                                ...
                                if (isActive) ...[
                                  ...
                                  SizedBox(
                                    ...
                                    child: OutlinedButton(
                                      onPressed: () => _requestTermination(c['_id']),
